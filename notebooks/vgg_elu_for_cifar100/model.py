...
def inference(images, NUM_CLASSES, training=True):
    ...
    if training==True:
      dropout_1 = tf.nn.dropout(pool1, 0.5)
    else:
    ...
    if training==True:
      dropout_2 = tf.nn.dropout(pool2, 0.5)
    else:
    ...
    if training==True:
      dropout_3 = tf.nn.dropout(pool3, 0.5)
    else:
    ...
    with tf.variable_scope('fc_3') as scope:
        ...
        weights = tf_variable_l2('weights', shape=[dim, 2048], stddev=0.05, wd=0.0005)
        biases = tf_variable('biases', [2048], tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('fc_4') as scope:
        weights = tf_variable_l2('weights', shape=[2048, 2048], stddev=0.05, wd=0.0005)
        biases = tf_variable('biases', [2048], tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = tf_variable_l2('weights', [2048, NUM_CLASSES], stddev=1/1024.0, wd=0.0)
