...
def inference(images, NUM_CLASSES, training=True):
    ...
    with tf.variable_scope('conv_pool1') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(conv1_2, kernel, [1, 2, 2, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv_pool1 = tf.nn.relu(pre_activation, name=scope.name)

    if training==True:
        dropout_1 = tf.nn.dropout(conv_pool1, 0.75)
    else:
        dropout_1 = tf.nn.dropout(conv_pool1, 1)
    ...
    with tf.variable_scope('conv_pool2') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(conv2_2, kernel, [1, 2, 2, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [128], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv_pool2 = tf.nn.relu(pre_activation, name=scope.name)

    if training==True:
        dropout_2 = tf.nn.dropout(conv_pool2, 0.75)
    else:
        dropout_2 = tf.nn.dropout(conv_pool2, 1)
    ...
    with tf.variable_scope('conv_pool3') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(conv3_3, kernel, [1, 2, 2, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv_pool3 = tf.nn.relu(pre_activation, name=scope.name)

    if training==True:
        dropout_3 = tf.nn.dropout(conv_pool3, 0.75)
    else:
        dropout_3 = tf.nn.dropout(conv_pool3, 1)
